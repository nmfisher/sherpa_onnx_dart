...
void main() {
  ...
  test('canRead returns false when no data written', () {
    final rBuffer = RingBuffer(readSizeInSamples:1, lengthInBytes:8);
  ...
  test('write/read data to the buffer with wraparound', () {
   ...
   test('different size writes/reads data to the buffer with wraparound', () {
    ...
    final rBuffer = RingBuffer(readSizeInSamples:2, lengthInBytes:8);
  ...
  test('write/read from entire file', () {
    final pcmData = File("test/test.pcm").readAsBytesSync();
    final rBuffer = RingBuffer(readSizeInSamples:pcmData.length ~/ 2, lengthInBytes:pcmData.length);
    rBuffer.write(pcmData);
    expect(rBuffer.canRead(), true);
    final pointer = rBuffer.read();

    outfile.writeAsBytesSync(pointer.asTypedList(pcmData.length ~/ 2).buffer.asUint8List());

  });

  test('write from entire file & read back in chunks', () {
    final pcmData = File("test/test.pcm").readAsBytesSync();
    final rBuffer = RingBuffer(readSizeInSamples:1024, lengthInBytes:pcmData.length);
    rBuffer.write(pcmData);
  ...
  test('write file in chunks & read back in entirety', () {
    ...
    final rBuffer = RingBuffer(readSizeInSamples:pcmData.length ~/ 2, lengthInBytes:pcmData.length * 2);
